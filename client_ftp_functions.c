...
void command_lls(char *lpwd)
{
	...
	struct dirent *e;
	while (e = readdir(d))
		printf("\t%s\t%s\n", e->d_type == 4 ? "DIR:" : e->d_type == 8 ? "FILE:"
																	  : "UNDEF",
			   e->d_name);
	closedir(d);
}
...
void command_lrm(char *path)
{
	...
	if (stat(path, &path_stat) == 0)
	{
		...
		else if (S_ISDIR(path_stat.st_mode))
		{
			if (rmdir(path) == 0)
			{
				printf("\tSuccessfully remove a directory.\n");
			}
		}
		else
		{
	...
	{
		printf("\tFile or Directory do not exist.\n");
	}
}

void command_rm(struct packet *chp, struct packet *data, int sfd_client, char *dirname)
{
	int x;
	set0(chp);
	chp->type = REQU;
	...
	if ((x = recv(sfd_client, data, size_packet, 0)) <= 0)
		er("recv()", x);
	chp = ntohp(data);
	if (chp->type == INFO && chp->comid == RM)
	{
		if (!strcmp(chp->buffer, "success"))
			printf("\tSuccessfully delete on server.\n");
		else if (!strcmp(chp->buffer, "fail"))
			printf("\tUnccessfully delete on server.\n");
	}
	else
		fprintf(stderr, "\tError executing command on the server.\n");
}
